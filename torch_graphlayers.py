...
from torchgraph_layers import Layer, LayerSplitter
from torchgraph_layers import convert_to_layers


class WeaveLayer(Layer):
  """ TorchGraph style implementation
  ...
  def create_tensor(self, in_layers=None, set_tensors=True, **kwargs):
    ...
    if self.update_pair:
      AP_ij = torch.matmul(
          torch.reshape(
              torch.gather(atom_features, atom_to_pair),
              [-1, 2 * self.n_atom_input_feat]), self.W_AP) + self.b_AP
      AP_ij = activation(AP_ij)
      AP_ji = torch.matmul(
          torch.reshape(
              torch.gather(atom_features, torch.transpose(atom_to_pair, [1])),
              [-1, 2 * self.n_atom_input_feat]), self.W_AP) + self.b_AP
      AP_ji = activation(AP_ji)

      PP = torch.matmul(pair_features, self.W_PP) + self.b_PP
      PP = activation(PP)
      P = torch.matmul(torch.cat([AP_ij + AP_ji, PP], 1), self.W_P) + self.b_P
      P = activation(P)
    else:
      P = pair_features
...
class WeaveGather(Layer):
  ...
  def gaussian_histogram(self, x):
    ...
    dist = [
        torch.distributions.normal(p[0], p[1])
...
class DTNNStep(Layer):
  ...
  def create_tensor(self, in_layers=None, set_tensors=True, **kwargs):
    ...
    outputs = torch.multiply(distance_hidden,
                          torch.gather(atom_features_hidden,
...
class DAGLayer(Layer):
  ...
  def create_tensor(self, in_layers=None, set_tensors=True, **kwargs):
    ...
    graph_features_initial = torch.zeros((self.max_atoms * self.batch_size,
                                       self.max_atoms + 1, self.n_graph_feat))
    # initialize graph features for each graph
    # another row of zeros is generated for padded dummy atoms
    graph_features = model_ops.Variable(graph_features_initial, trainable=False)
    ...
    for count in range(self.max_atoms):
          ...
          [
              torch.reshape(
                  torch.stack(
                      ...
                      (self.max_atoms - 1),
                      dim=1), [-1]),
              torch.reshape(torch.masked_select(parents[:, count, 1:], mask), [-1])
          ],
          dim=1)
      ...
      target_index = torch.stack([torch.arange(n_atoms), parents[:, count, 0]], axis=1)
      target_index = torch.masked_select(target_index, mask)
      # update the graph features for target atoms
      graph_features = torch.Tensor.scatter_(graph_features, target_index,
                                            batch_outputs)
  ...
  def DAGgraph_step(self, batch_inputs, W_list, b_list, **kwargs):
    ...
    for idw, W in enumerate(W_list):
      outputs = torch.matmul(outputs, W) + b_list[idw]
      outputs = self.activation(outputs)
      training = kwargs['training'] if 'training' in kwargs else 1.0
      if not self.dropout is None:
        p = (self.dropout * training)
        outputs = F.Dropout(outputs, p)
    return outputs
...
class DAGGather(Layer):
  ...
  def DAGgraph_step(self, batch_inputs, W_list, b_list, **kwargs):
    ...
    for idw, W in enumerate(W_list):
      ...
      training = kwargs['training'] if 'training' in kwargs else 1.0
      if not self.dropout is None:
        outputs = F.dropout(outputs, p = (self.dropout * training))
    return outputs
...
class MessagePassing(Layer):
  ...
  def create_tensor(self, in_layers=None, set_tensors=True, **kwargs):
    ...
    if n_atom_features < self.n_hidden:
      ...
      out = F.pad(atom_features, ((0, 0), (0, pad_length)), mode='constant')
...
class EdgeNetwork(object):
  ...
  def forward(self, atom_features, atom_to_pair):
    out = torch.unsqueeze(torch.gather(atom_features, atom_to_pair[:, 1]), 2)
    out = torch.squeeze(torch.matmul(self.A, out), dim=2)
    out = torch.sum(out, atom_to_pair[:, 0])
    return out
...
class GatedRecurrentUnit(object):
  ...
  def forward(self, inputs, messages):
    z = torch.sigmoid(torch.matmul(messages, self.trainable_weights[0]) + \
                      torch.matmul(inputs, self.trainable_weights[3]) + \
                      self.trainable_weights[6])
    r = torch.sigmoid(torch.matmul(messages, self.trainable_weights[1]) + \
                      torch.matmul(inputs, self.trainable_weights[4]) + \
                      self.trainable_weights[7])
    h = (1 - z) * nn.Tanh(torch.matmul(messages, self.trainable_weights[2]) + \
                             torch.matmul(inputs * r, self.trainable_weights[5]) + \
...
class SetGather(Layer):
  ...
  def create_tensor(self, in_layers=None, set_tensors=True, **kwargs):
    ...
    for i in range(self.M):
      ...
      e_mols = [
          torch.cat([e_mol, nn.init.constant_([-1000.])], 0) for e_mol in e_mols
      ]
      a = torch.cat([F.softmax(e_mol)[:-1] for e_mol in e_mols], 0)
      r = torch.sum(torch.reshape(a, [-1, 1]) * atom_features, atom_split)
      # Model using this layer must set pad_batches=True
  ...
  def LSTMStep(self, h, c, x=None):
    ...
    z = torch.matmul(h, self.U) + self.b
    i = F.sigmoid(z[:, :self.n_hidden])
    f = F.sigmoid(z[:, self.n_hidden:2 * self.n_hidden])
    o = F.sigmoid(z[:, 2 * self.n_hidden:3 * self.n_hidden])
    z3 = z[:, 3 * self.n_hidden:]
    c_out = f * c + i * nn.Tanh(z3)
    h_out = o * nn.Tanh(c_out)
