...
from torch_layers import Label, Feature, Weights, Constant, Input
from torch_modelops import create_variable
from torch_optimizers import Adam
...
class TorchGraph(Model):
  ...
  def __init__(self,
    ...
    self.optimizer = Adam(
        lr=lr, betas=(0.9, 0.999), epsilon=1e-7)
    ...
    super(TorchGraph, self).__init__(**kwargs)
    ...
    if self.use_queue and self.tensorboard:
      raise ValueError(
          "Currently TorchGraph cannot both use_queue and tensorboard at the same time"
      )
  ...
  def fit_generator(self,
    ...
    if not self.built:
      self.build()
    with self._get_torch("Graph").as_default():
      time1 = time.time()
      loss = self.loss
      ...
      if torch.cuda.is_available():
        ...
        submodel_vars = None
        if submodel is None:
          optimizer = self._get_torch("Optimizer")
        else:
        ...
        if submodel is None:
          train_op = self._get_torch('train_op')
        else:
          train_op = submodel.get_train_op()
      if checkpoint_interval > 0:
        saver = torch.save(
            self.get_variables(),
            obj=max_checkpoints_to_keep)
      if restore:
        self.restore()
      ...
      if self.queue_installed:
        enqueue_thread = threading.Thread(
            ...
            args=(self, feed_dict_generator, self._get_torch("Graph"),
      ...
      for feed_dict in self._create_feed_dicts(feed_dict_generator, True):
        ...
        if torchvision.models.resnet():
          ...
          avg_loss += value
            ...
            if should_log:
              fetches.append(self._get_torch("summary_op"))
  ...
  def _log_tensorboard(self, summary):
    ...
    writer = self._get_torch("FileWriter")
    writer.reopen()
  ...
  def _predict(self, generator, transformers, outputs, uncertainty):
    ...
    ----------
    generator: Generator
      Generator that constructs feed dictionaries for TorchGraph.
    transformers: list
    ...
    with self._get_torch("Graph").as_default():
      ...
      if self.queue_installed:
        enqueue_thread = threading.Thread(
            target=_enqueue_batch,
            args=(self, generator, self._get_torch("Graph"), self.session,
  ...
  def build(self):
    ...
    with self._get_torch("Graph").as_default():
      self._training_placeholder = torch.int32
  ...
  #      tf.summary.tensor_summary(layer.name, layer.out_tensor)
    if self.tensorboard:
      writer = self._get_torch("FileWriter")
      writer.add_graph(self._get_torch("Graph"))
      writer.close()
  ...
  def create_submodel(self, layers=None, loss=None, optimizer=None):
    """Create an alternate objective for training one piece of a TorchGraph.

    A TorchGraph consists of a set of layers, and specifies a loss function and
  ...
  def get_global_step(self):
    return self._get_torch("GlobalStep")

  def _get_torch(self, obj):
    """Fetches underlying PyTorch primitives.

    Parameters
    ----------
    obj: str
      If "Graph", returns tf.Graph instance. If "FileWriter", returns
      tensorboard-pytorch.FileWriter. If "Optimizer", returns the optimizer. If
    ...
    if obj == "Graph":
      self.tensor_objects['Graph'] = torch.Graph()
    # Must download tensorboardX for this to work
    elif obj == "FileWriter":
      self.tensor_objects['FileWriter'] = FileWriter(self.model_dir)
    elif obj == 'Optimizer':
      self.tensor_objects['Optimizer'] = self.optimizer._create_optimizer(
          self._get_torch('GlobalStep'))
    elif obj == 'train_op':
      opt = self._get_torch('Optimizer')
      global_step = self._get_torch('GlobalStep')
      try:
  ...
  def load_from_dir(model_dir, restore=True):
    ...
    with open(pickle_name, 'rb') as fout:
      torchgraph = pickle.load(fout)
      torchgraph.built = False
      torchgraph.model_dir = model_dir
      if restore:
        try:
          torchgraph.restore()
        except ValueError:
  ...
  #                   model_dir=None,
  #                   config=None):
    """Construct a PyTorch Estimator from this model.
  ...
  def to_sparse(x):
      """Converts a dense tensor x to a sparse format"""
      x_typename = torch.typename(x).split('.')[-1]
      sparse_tensortype = getattr(torch.sparse. x_typename)

      indices = torch.nonzero(x)
      if len(indices.shape) == 0: #If all elements are zero
  ...
  def create_estimator_inputs(self, feature_columns, weight_column, features,
    ...
    method.
    """
    if self.__class__.default_generator != TorchGraph.default_generator:
      raise ValueError(
          "Class overrides default_generator() but not create_estimator_inputs()"
      )
    tensors = {}
    for layer, column in zip(self.features, feature_columns):
      tensors[layer] = torch.sparse.FloatTensor(i, v).to_dense(features, [column])
    if weight_column is not None:
      tensors[self.task_weights[0]] = torch.sparse.FloatTensor(i,v).to_dense(
          features, [weight_column])
    if labels is not None:
      tensors[self.labels[0]] = torch.Tensor.type(labels, self.labels[0].dtype)
    return tensors
...
class TorchWrapper(object):
  """This class exists as a workaround for Torch objects not being picklable.
  ...
  def __init__(self, torch_class, **kwargs):
    """Create a TorchWrapper for constructing a PyTorch object.
  ...
  def __call__(self):
    return self.torch_class(**self.kwargs)


class Submodel(object):
  """An alternate objective for training one piece of a TorchGraph."""

  def __init__(self, graph, layers, loss, optimizer):
    """Create a submodel.

    In normal use, you should call create_submodel() on the TorchGraph instead
  ...
  def get_train_op(self):
    ...
    if self._train_op is None:
      ...
      if self.loss is None:
        ...
        loss = self.loss
      torch_opt = self.create_optimizer()
      global_step = self.graph._get_torch('GlobalStep')
      self._train_op = torch_opt.minimize(loss.out_tensor, global_step, variables)
    return self._train_op

  def create_optimizer(self):
    """Create the PyTorch optimizer to use for training."""
