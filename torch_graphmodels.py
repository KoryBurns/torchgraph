...
from torchgraph_graphlayers import WeaveGather, \
    DTNNEmbedding, DTNNStep, DTNNGather, DAGLayer, \
    DAGGather, DTNNExtract, MessagePassing, SetGather
from torchgraph_graphlayers import WeaveLayerFactory
from torchgraph_layers import Layer, Dense, SoftMax, Reshape, \
...
from torchgraph_layers import L2Loss, Label, Weights, Feature
...
class TrimGraphOutput(Layer):
  ...
  def create_tensor(self, in_layers=None, set_tensors=True, **kwargs):
    ...
    n_samples = torch.Size(inputs[1])[0]
...
class WeaveModel(TorchGraph):
  ...
  def build_graph(self):
    ...
    batch_norm1 = BatchNorm(eps=1e-5, in_layers=[dense1])
...
class DTNNModel(TorchGraph):
  ...
  def build_graph(self):
    """Building graph structures:
            ...
            """
    self.atom_number = Feature(shape=(None,), dtype=torch.int32)
    self.distance = Feature(shape=(None, self.n_distance))
    self.atom_membership = Feature(shape=(None,), dtype=torch.int32)
    self.distance_membership_i = Feature(shape=(None,), dtype=torch.int32)
    self.distance_membership_j = Feature(shape=(None,), dtype=torch.int32)
...
class DAGModel(TorchGraph):
  ...
  def build_graph(self):
    ...
    self.parents = Feature(
        shape=(None, self.max_atoms, self.max_atoms), dtype=torch.int32)
    self.calculation_orders = Feature(
        shape=(None, self.max_atoms), dtype=torch.int32)
    self.calculation_masks = Feature(
        shape=(None, self.max_atoms), dtype=bool)
    self.membership = Feature(shape=(None,), dtype=torch.int32)
    self.n_atoms = Feature(shape=(), dtype=torch.int32)
    dag_layer1 = DAGLayer(
...
class GraphConvModel(TorchGraph):
  ...
  def build_graph(self):
    ...
    self.degree_slice = Feature(shape=(None, 2), dtype=torch.int32)
    self.membership = Feature(shape=(None,), dtype=torch.int32)
    ...
    for layer_size, dropout in zip(self.graph_conv_layers, self.dropout):
      ...
      gc1 = GraphConv(layer_size, activation_fn=nn.ReLU, in_layers=gc1_in)
...
class MPNNModel(TorchGraph):
  ...
  def build_graph(self):
    ...
    self.atom_features = Feature(shape=(None, self.n_atom_feat))
    self.pair_features = Feature(shape=(None, self.n_pair_feat))
    self.atom_split = Feature(shape=(None,), dtype=torch.int32)
    self.atom_to_pair = Feature(shape=(None, 2), dtype=torch.int32)
...
#################### Deprecation warnings for renamed TorchGraph models ####################

import warnings

TORCHGRAPH_DEPRECATION = "{} is deprecated and has been renamed to {} and will be removed in TorchGraph 1.0."
...
class GraphConvTensorGraph(GraphConvModel):
  ...
  def __init__(self, *args, **kwargs):
    ...
    warnings.warn(
        TORCHGRAPH_DEPRECATION.format("GraphConvTensorGraph",
...
class WeaveTensorGraph(WeaveModel):
  ...
  def __init__(self, *args, **kwargs):
    ...
    warnings.warn(
        TORCHGRAPH_DEPRECATION.format("WeaveTensorGraph", "WeaveModel"),
        FutureWarning)
...
class DTNNTensorGraph(DTNNModel):
  ...
  def __init__(self, *args, **kwargs):
    ...
    warnings.warn(
        TORCHGRAPH_DEPRECATION.format("DTNNTensorGraph", "DTNNModel"),
        FutureWarning)
...
class DAGTensorGraph(DAGModel):
  ...
  def __init__(self, *args, **kwargs):
    ...
    warnings.warn(
        TORCHGRAPH_DEPRECATION.format("DAGTensorGraph", "DAGModel"),
        FutureWarning)
...
class MPNNTensorGraph(MPNNModel):
  ...
  def __init__(self, *args, **kwargs):
    ...
    warnings.warn(
        TORCHGRAPH_DEPRECATION.format("MPNNTensorGraph", "MPNNModel"),
        FutureWarning)
